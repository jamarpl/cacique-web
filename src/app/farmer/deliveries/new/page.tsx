...
/**
 * UIRB-FE-F7. Farmer-scoping: the candidate crate list comes from
 * `GET /api/inventory/allocatable?farmerId=` (own crates only) and the
 * optional order picker from `GET /api/orders?farmerId=` (own orders only)
 * — both scoped to the acting farmer, so this screen can never select
 * another farmer's crates or attach another farmer's order.
 *
 * Endpoint-shape note: `CreateDeliveryRequest` has no `buyerId` field — a
 * delivery's "customer" is only implied indirectly via an optional
 * `orderId` (Order -> Buyer). There is also no warehouse picker; the server
 * derives the delivery's warehouse from the selected crates' current
 * warehouse and requires them to all match.
 */
export default function RegisterDeliveryPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      inventoryApi.listAllocatable({ farmerId }, controller.signal),
      ordersApi.list({ farmerId, ...(buyerIdFilter ? { buyerId: buyerIdFilter } : {}) }, controller.signal),
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Register delivery</CardTitle>
        <CardDescription>
          Pick your crates, an optional order to fulfil, and a driver/vehicle. The pickup warehouse is
          determined automatically from the crates you select — all must currently be at the same warehouse.
        </CardDescription>
      </CardHeader>
      <CardContent>
        {loading ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  {crates.map((crate) => (
                    ...
                    >
                      <span className="flex items-center gap-2">
                        ...
                        />
                        <span>
                          {crate.cropName} · {crate.weight}kg · Grade {crate.grade}
                        </span>
                      </span>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="space-y-2">
                <Label htmlFor="delivery-driver">Driver</Label>
                <Select value={driverId} onValueChange={(v) => setDriverId(v ?? "")}>
                  <SelectTrigger id="delivery-driver" className="w-full">
              ...
              <div className="space-y-2">
                <Label htmlFor="delivery-vehicle">Vehicle</Label>
                <Select value={vehicleId} onValueChange={(v) => setVehicleId(v ?? "")}>
                  <SelectTrigger id="delivery-vehicle" className="w-full">
              ...
              <div className="space-y-2">
                <Label htmlFor="delivery-departure">Departure time (optional)</Label>
                <Input
